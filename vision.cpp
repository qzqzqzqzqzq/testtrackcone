...
void updateTargetRoute(const cv::Mat& frame_clone)
{
    ...
    //
    std::pair<int, int> cone_left_right = dect_cone(mask_cone, cropped_image);// 新版
    int cone_left = cone_left_right.first;
    int cone_right = cone_left_right.second;
    // +++ 新增调试显示部分 +++
    if (ConeInformation.debug_enabled) {
        // 显示带有矩形标注的裁剪图像（类似原来的 “frame_a”）
        cv::imshow("Cone Debug", cropped_image);

        // 显示锥桶的二值掩膜图像（类似原来的 “blue”）
        cv::imshow("Cone Mask", mask_cone);
...
std::pair<int, int> dect_cone(cv::Mat& mask_cone, cv::Mat& debug_image)
{
    ...
    if (!Cones.empty()) {
        ...
        if (ConeInformation.debug_enabled) {
            // 绘制白色矩形框和绿色 "cone" 标签
            cv::rectangle(debug_image, rect, cv::Scalar(255, 255, 255), 2);
            cv::putText(debug_image, "cone", cv::Point(rect.x, rect.y - 5),
